function [particleFilter,weight_sum] = particleFiltering(beta, roundCount,sigma,K, numObservations, observations, robotPoses, srcGroundTruth,image,row, col,resolution)
    ...
    while true
        ...
        t = t + 1;
        particles = particles + randn(numParticles,2)*(0.01/resolution); % add small Guassian noise to last state
        ...
        for i = 1:numParticles % For each particle
            ...
            L2 = exp(-d/beta); % Add penalty
            % L2 = 1; % WDP

            for j = 1:numObservations 
                if ~isnan(observations(t,j)) % If the observation is not NaN
                    angle = wrapToPi(observations(t,j) + deg2rad(robotPoses(t,4)));  % Convert azimuth to the world coordinate system, in radians
           ...
           likelihood = max(lamda,exp(-0.5 * (angleDiff / sigma)^2) / (sqrt(2*pi) * sigma)*L2); 
           % likelihood = exp(-0.5 * (angleDiff / sigma)^2) / (sqrt(2*pi) * sigma); % original PDF
        ...
        if covariances < 0.1 || stepCount == 2*K  
            ...
            disp(['Exit filtering, stepCount = ',num2str(stepCount)])
            break
        end
